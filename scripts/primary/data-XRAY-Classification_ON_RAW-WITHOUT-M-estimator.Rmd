...
run_locally = F
if(run_locally){
GRBData_raw_data_w_errorbar <- read.csv(file = "OnlyLGRBs_data_171.csv",
                                        header=TRUE)
cutoff=3.0} else {
...
```{r}
nrow(GRBData_raw_data_w_errorbar[GRBData_raw_data_w_errorbar$z > 3,])
#nrow(GRBData10[GRBData10$z > cutoff,])
```

# Transformation of the variables
```{r}
GRBData_raw_1 <- data.frame(matrix(ncol = 0, nrow = nrow(GRBData_raw_data_w_errorbar)))
GRBData_raw_1$GRB_Name <- GRBData_raw_data_w_errorbar$Name
GRBData_raw_1$z <- GRBData_raw_data_w_errorbar$z
GRBData_raw_1$log10T90 <- log10(GRBData_raw_data_w_errorbar$T90)
GRBData_raw_1$log10Fluence <- log10(GRBData_raw_data_w_errorbar$Fluence*(10^(-7)))
GRBData_raw_1$log10PeakFlux <- log10(GRBData_raw_data_w_errorbar$PeakFlux)
GRBData_raw_1$PhotonIndex <- GRBData_raw_data_w_errorbar$PhotonIndex
#GRBData_raw_1$log10NH <- log10(GRBData_raw_data_w_errorbar$NH)
GRBData_raw_1$log10Fa <- GRBData_raw_data_w_errorbar$logFa
GRBData_raw_1$log10Ta <- GRBData_raw_data_w_errorbar$logT_a
GRBData_raw_1$log10NH <- log10(GRBData_raw_data_w_errorbar$NH*(10^21))
...
GRBData_raw_1$log10PeakFluxErr <- GRBData_raw_data_w_errorbar$PeakFluxErr / ((10**GRBData_raw_1$log10PeakFlux) * log(10))  ## Defining the Fluence error in log values
GRBData_raw_1$PhotonIndexErr <- GRBData_raw_data_w_errorbar$PhotonIndexErr
GRBData_raw_1$log10FaErr <- GRBData_raw_data_w_errorbar$logFaErr
GRBData_raw_1$log10TaErr <- GRBData_raw_data_w_errorbar$logTaErr
...
#GRBData_raw_1$log10FluenceErr[GRBData_raw_1$log10Fluence == 0] <- NA
GRBData_raw_1$Alpha[GRBData_raw_1$Alpha > 3] <- NA
GRBData_raw_1$AlphaErr[is.na(GRBData_raw_1$Alpha)] <- NA
...
```{r}
str(GRBData_raw_1)
write.csv(GRBData_raw_1, "GRBData_raw_1.csv", row.names = FALSE)
```


# Creating a new data with removed NA values
```{r}
GRBData_raw_2 <- na.omit(GRBData_raw_1)  # Omit NA values from the data
## Adding log10(z+1) as extra variable ##
write.csv(GRBData_raw_1, "GRBData_raw_2.csv", row.names = FALSE)
...
GRBData2 <- GRBData1[GRBData1$log10PeakFlux_error_bar == 0,]
GRBData3 <- GRBData2[GRBData2$log10PhotonIndex_error_bar == 0,]
GRBData6 <- GRBData3[GRBData3$log10Fa_error_bar == 0,]
GRBData7 <- GRBData6[GRBData6$log10Ta_error_bar == 0,]
GRBData4 <- GRBData7[GRBData7$Alpha_error_bar == 0,]
GRBData5 <- GRBData4[GRBData4$Beta_error_bar == 0,]

print(paste("After removing pairs outliers", "GRBData2:", nrow(GRBData2), " |", "GRBData3:", nrow(GRBData3), " |", "GRBData4:", nrow(GRBData4), " |", "GRBData5:", nrow(GRBData5)))
```
...
#GRBData10 <- GRBData5[,-c(1,13:20,22:29)]
GRBData10 <- GRBData5[,-c(1,12:17,19:24)]
...
#cols <- colnames(GRBData10)
print(cols)
cols_1 <- colnames(TrainingData[,-c(1,11,12,13)])
#cols_1 <- colnames(TrainingData)
print(cols_1)
```
