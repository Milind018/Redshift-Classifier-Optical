...
PLOTaddr <- "CURRENT_ANALYSIS/Graphics/XRAY/MICE_WITHOUT_M-estimator/"

## CREATES THE FOLDERS WHERE RESULTS ARE KEPT ##

if(!dir.exists('CURRENT_ANALYSIS/Graphics/XRAY/MICE_WITHOUT_M-estimator')){dir.create('CURRENT_ANALYSIS/Graphics/XRAY/MICE_WITHOUT_M-estimator')}
```
...
run_locally = F
if(run_locally){
GRBData_raw_data_w_errorbar <- read.csv(file = "CURRENT_ANALYSIS/XRAY/MICE/Without_Mestimator/NEW-XRAY_DATA_with_errorbar_after_MICE_ON_RAW_without_M-estimator.csv", header = TRUE)
cutoff=3.0} else {
  args <- commandArgs(trailingOnly = TRUE)
  cutoff <- as.numeric(args[1])
  GRBData_raw_data_w_errorbar <- read.csv(file = "OnlyLGRBs_data_171.csv", header = TRUE)}
```
...
```{r}
libs_SL_CV = c("SL.kernelKnn", "SL.lda", "SL.qda", "SL.ranger", "SL.randomForest")
loop <- 100
...
#set.seed(12345)
system.time({
  numCores = detectCores() - 4 # Leaving two cores out so you can still use your machine or if you use the cluster then use all the cores
  registerDoParallel(numCores)
  loop = 100
