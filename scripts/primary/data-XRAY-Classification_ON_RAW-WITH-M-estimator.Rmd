...
PLOTaddr <- "CURRENT_ANALYSIS/Graphics/XRAY/RAW_WITH_M-estimator/"

## CREATES THE FOLDERS WHERE RESULTS ARE KEPT ##

if(!dir.exists('CURRENT_ANALYSIS/Graphics/XRAY/RAW_WITH_M-estimator')){dir.create('CURRENT_ANALYSIS/Graphics/XRAY/RAW_WITH_M-estimator')}
```
...
numCores = detectCores() -2 # Leave two cores out so you can still use your machine or if you use the cluster then use all the cores
...
run_locally = F
if(run_locally){
GRBData_raw_data_w_errorbar <- read.csv(file = "CURRENT_ANALYSIS/XRAY/M-estimator/TEST/NEW-XRAY_DATA_useable_w_errorbar_after_M-estimator_ON_RAW.csv", header = TRUE)
cutoff=3.0} else {
...
system.time({
  ...
  loop = 2  # Looping the SL over 100 times to compute the prediction error, we don't need to use the pacakges for the list of models in .packages because they are already inside the SuperLearner package
...
#print(libs_SL_CV)
libs_SL_CV = c('SL.cforest','SL.kernelKnn', 'SL.qda', 'SL.earth')
```
...
legend(x = 0.52, y = 0.28, legend = legend_text, col = ROCpal, lwd = 30, cex = 1, text.font = 2, bty = "n")   # set manually the position of the legend here
# Draw thick border around legend box, set manually here the position of the box around the legend
rect(0.52, -0.023, -0.12, 0.265,
     border = "black", lwd = 15)
